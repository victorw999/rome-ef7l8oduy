...
export default class UnitMilitary implements Unit {
  ...
  private getRandomDamage(target: Unit): number {
    ...
    // If the attack power is less than or equal to defence, reduce the probability of higher damage
      ...
      const baseDamage =
        Math.random() < penaltyFactor
          ? Math.floor(Math.random() * this.attack_power)
          : 0;
  ...
  status: StringFormat = function (msg: string, upper: boolean): string {
    console.log('====> this.name', this.name);
    let status = ` ${this.name} ${this.size} `;
    status = upper ? status.toUpperCase() : status;
    return status + msg;
  };

  // status: StringFormat = (msg:string, upper:boolean): string => {
  //   let status = ` ${this.name} ${this.size} `
  //   status = upper ? status.toUpperCase() : status
  //   return status + msg
  // }
